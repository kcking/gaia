...
TOOLCHAIN_TO_OS = {
    "@zig_sdk//toolchain:linux_amd64_gnu.2.19": struct(os = "linux", arch = "x86_64"),
    ...
    "@zig_sdk//toolchain:windows_amd64": struct(os = "windows", arch = "x86_64"),
...
def _zig_toolchains_impl(repository_ctx):
    ...
    toolchains = [tc for tc in TOOLCHAIN_TO_OS if not (getattr(TOOLCHAIN_TO_OS[tc], "os") == os and getattr(TOOLCHAIN_TO_OS[tc], "arch") == arch)]
